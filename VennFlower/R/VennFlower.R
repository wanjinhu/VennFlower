...
#'   "#6181BD4E", "#FDA1004E"
#' )
#' df <- read.table("../gene_presence_absence.Rtab", header = T, row.names = 1)
#' png("venn_flower.png", width = 10, height = 10, units = "in", res = 300)
#' p <- venn.flower(df = df, ellipse_col = my_col)
#' dev.off()
venn.flower <- function(df,
  ...
  uniq_num <- sapply(sample, function(i) {
    nrow(df_binary %>% filter(df_binary[[i]] == 1 & total == 1))
  })
  df_core <- df_binary %>% filter(total == length(sample))
